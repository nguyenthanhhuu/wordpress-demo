...
<tr>
						<td colspan="15">
							<p class="description"><?php _e( 'Choose icons from Font Awesome Pack', STM_DOMAIN ); ?></p>
							<input type="text" class="stm_woo_category_icon_search" />
						</td>
					</tr>
-->
					<tr>
	...
	<script type="text/javascript">
		jQuery(function($) {
		    $(function() {
				...
				$( ".stm_woo_category_icon_search" ).keypress(function() {
					var searchVal = $(this).val();
					$(this).closest('.stm_theme_icon_font_table').find('i').each(function(){
						var substring_search = $(this).attr('data-search');
					});
				});
		    });
...
if(function_exists('icl_register_string')) {
	...
	function woo_custom_cart_button_text() {
	 	icl_register_string('WooTranslate', 'Favorite', 'Favorite');
	    $str = icl_t('WooTranslate', 'Favorite', 'Favorite');
	}
} else {
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'woo_custom_cart_button_text' );    // 2.1 +
	...
	function woo_custom_cart_button_text() {
		...
		return __( 'Take this course', 'woocommerce' );
